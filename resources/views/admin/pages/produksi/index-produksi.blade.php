...
<div class="container">
    <div class="page-header">
        ...
        <a href="{{ route('backoffice.produksi.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Produksi</a>
    </div>
    ...
    <table class="table" id="dataTable">
        <thead>
            <tr>
                ...
                <th data-sort="produk" style="width: 20%;">Produk
                    <span class="sort-icons">
                        <i class="fas fa-sort-up sort-up"></i>
                        <i class="fas fa-sort-down sort-down"></i>
                    </span>
                </th>
                <th data-sort="tanggal" style="width: 12%;">Tanggal Produksi
                    <span class="sort-icons">
                        <i class="fas fa-sort-up sort-up"></i>
                        <i class="fas fa-sort-down sort-down"></i>
                    </span>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ number_format($produksi->jumlah_hasil, 2) }}</td>
                <td>
                    <span class="badge {{ in_array($produksi->status, ['selesai', 'diproses']) ? 'badge-success' : 'badge-danger' }}">
                        {{ $produksi->status_label }}
                    </span>
                </td>
                <td class="actions">
                    <button class="btn btn-info btn-action" title="Lihat" onclick="viewData(event)"><i class="fas fa-eye"></i></button>
                    <a href="{{ route('backoffice.produksi.edit', $produksi->id) }}" class="btn btn-warning btn-action" title="Edit"><i class="fas fa-edit"></i></a>
                    <button type="button" class="btn btn-danger btn-action delete-btn" title="Hapus" 
                            data-id="{{ $produksi->id }}" 
                            data-nama="{{ $produksi->nomor_produksi }}"
    ...
    <div class="pagination">
        ...
        <div class="pagination-buttons">
            ...
            @else
                <a href="{{ $produksis->previousPageUrl() . (request('per_page') ? '&per_page=' . request('per_page') : '') }}"><button><i class="fas fa-chevron-left"></i></button></a>
            @endif
            ...
            @if($produksis->hasMorePages())
                <a href="{{ $produksis->nextPageUrl() . (request('per_page') ? '&per_page=' . request('per_page') : '') }}"><button><i class="fas fa-chevron-right"></i></button></a>
            @else
...
<script>
    ...
    function sortTable(column) {
        ...
        rows.sort((a, b) => {
            ...
            } else if (column === 'target' || column === 'hasil') {
                aValue = parseFloat(a.cells[colIndex].textContent.replace(/,/g, ''));
                bValue = parseFloat(b.cells[colIndex].textContent.replace(/,/g, ''));
            } else {
