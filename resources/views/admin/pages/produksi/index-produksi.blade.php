...
@section('content')
    <style>
        ...
        * {
        ...
        .table th .sort-icons i.sort-up {
            margin-bottom: -5px;
        }

        .table th .sort-icons i.sort-down {
            margin-top: -5px;
        }
        ...
        .search-box input:focus {
            ...
            width: 280px;
            /* expand on focus to match master-bahan */
        }
    ...
    <div class="container">
        <div class="page-header">
            <h1><i class="fas fa-cogs"></i> Manajemen Produksi</h1>
            <a href="{{ route('backoffice.produksi.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah
                Produksi</a>
        </div>
        ...
        <div class="controls">
            ...
            <div class="right-controls">
                <div class="search-box">
                    ...
                    <button type="button" class="clear-btn" onclick="clearSearch()" style="display:none;"><i
        ...
        <div class="table-responsive">
            <table class="table" id="dataTable">
                <thead>
                    <th data-sort="no" style="width: 5%;">No
                    ...
                    <th data-sort="batch" style="width: 15%;">Batch Produksi
                        <span class="sort-icons">
                            <i class="fas fa-sort-up sort-up"></i>
                            <i class="fas fa-sort-down sort-down"></i>
                        </span>
                    </th>
                    <th data-sort="produk" style="width: 10%;">Produk
                        <span class="sort-icons">
                            <i class="fas fa-sort-up sort-up"></i>
                            <i class="fas fa-sort-down sort-down"></i>
                        </span>
                    </th>
                    <th data-sort="tanggal" style="width: 15%;">Tanggal Produksi
                        <span class="sort-icons">
                            <i class="fas fa-sort-up sort-up"></i>
                            <i class="fas fa-sort-down sort-down"></i>
                        </span>
                    </th>
                    <th data-sort="target" style="width: 10%;">Target
                        <span class="sort-icons">
                            <i class="fas fa-sort-up sort-up"></i>
                            <i class="fas fa-sort-down sort-down"></i>
                        </span>
                    </th>
                    <th data-sort="hasil" style="width: 10%;">Hasil
                        <span class="sort-icons">
                            <i class="fas fa-sort-up sort-up"></i>
                            <i class="fas fa-sort-down sort-down"></i>
                        </span>
                    </th>
                    <th data-sort="biaya" style="width: 10%;">Biaya
                        <span class="sort-icons">
                            <i class="fas fa-sort-up sort-up"></i>
                            <i class="fas fa-sort-down sort-down"></i>
                        </span>
                ...
                <tbody>
                    @forelse($produksis ?? [] as $index => $produksi)
                        <tr>
                            ...
                            <td>{{ $produksi->batchProduksi->nomor_batch ?? '-' }}</td>
                            <td>{{ $produksi->produk->nama_produk ?? '-' }}</td>
                            <td>{{ $produksi->tanggal_produksi->format('d/m/Y') }}</td>
                            <td>{{ number_format($produksi->jumlah_target, 0) }}</td>
                            <td>{{ number_format($produksi->jumlah_hasil, 0) }}</td>
                            <td>Rp {{ number_format($produksi->biaya_produksi, 0, ',', '.') }}</td>
                            <td>
                                <span
                                    class="badge {{ in_array($produksi->status, ['selesai', 'proses']) ? 'badge-success' : 'badge-danger' }}">
                                    {{ $produksi->status_label }}
                                </span>
                            </td>
                            <td class="actions">
                                <button class="btn btn-info btn-action" title="Lihat" onclick="viewData(event)"><i
                                        class="fas fa-eye"></i></button>
                                <a href="{{ route('backoffice.produksi.edit', $produksi->id) }}"
                                    class="btn btn-warning btn-action" title="Edit"><i class="fas fa-edit"></i></a>
                                <button type="button" class="btn btn-danger btn-action delete-btn" title="Hapus"
                                    data-id="{{ $produksi->id }}" data-nama="{{ $produksi->nomor_produksi }}"
                                    data-url="{{ route('backoffice.produksi.destroy', $produksi->id) }}">
        ...
        @if (isset($produksis) && $produksis->hasPages() && $produksis->lastPage() > 1)
            <div class="pagination">
                <div class="pagination-info">
                    Menampilkan {{ $produksis->firstItem() ?? 0 }} sampai {{ $produksis->lastItem() ?? 0 }} dari
                    {{ $produksis->total() ?? 0 }} entri
                </div>
                <div class="pagination-buttons">
                    ...
                    @else
                        <a
                            href="{{ $produksis->previousPageUrl() . (request('per_page') ? '&per_page=' . request('per_page') : '') }}"><button><i
                    ...
                    @foreach ($produksis->getUrlRange(1, $produksis->lastPage()) as $page => $url)
                        ...
                        @else
                            <a
                                href="{{ $url . (request('per_page') ? '&per_page=' . request('per_page') : '') }}"><button>{{ $page }}</button></a>
                        @endif
                    @endforeach

                    @if ($produksis->hasMorePages())
                        <a
                            href="{{ $produksis->nextPageUrl() . (request('per_page') ? '&per_page=' . request('per_page') : '') }}"><button><i
        ...
        @elseif(isset($produksis) && $produksis->total() > 0)
            <div class="pagination">
                <div class="pagination-info">
                    Menampilkan {{ $produksis->firstItem() ?? 0 }} sampai {{ $produksis->lastItem() ?? 0 }} dari
                    {{ $produksis->total() ?? 0 }} entri
        ...
        @elseif(isset($produksis) && !method_exists($produksis, 'hasPages') && $produksis->count() > 0)
            <div class="pagination">
                <div class="pagination-info">
                    Menampilkan semua {{ $produksis->count() ?? 0 }} entri
                </div>
                <div class="pagination-buttons">
    ...
    <script>
        ...
        function filterData() {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                const nomorProduksi = rows[i].cells[1].textContent.toLowerCase();
                const batch = rows[i].cells[2].textContent.toLowerCase();
                const produk = rows[i].cells[3].textContent.toLowerCase();

                const matchesSearch = nomorProduksi.includes(searchText) ||
        ...
        function toggleClearButton() {
            ...
            if (!input || !btn) return;
            if (input.value.trim().length > 0) btn.style.display = 'block';
        ...
        function sortTable(column) {
            ...
            rows.sort((a, b) => {
                ...
                } else if (column === 'target' || column === 'hasil' || column === 'biaya') {
                    aValue = parseFloat(a.cells[colIndex].textContent.replace(/[^\d]/g, ''));
                    bValue = parseFloat(b.cells[colIndex].textContent.replace(/[^\d]/g, ''));
                } else {
        ...
        function viewData(event) {
            ...
            const nomorProduksi = cells[1].textContent;
            const batch = cells[2].textContent;
            const produk = cells[3].textContent;
            const tanggal = cells[4].textContent;
            const target = cells[5].textContent.replace(/,/g, '');
            const hasil = cells[6].textContent.replace(/,/g, '');
            const biaya = cells[7].textContent;
            ...
            <div class="detail-box">
                <div class="detail-header">
                    ...
                    <div class="flex-grow-1">
                        <div class="detail-title">${produk}</div>
                        <div class="detail-sub">${nomorProduksi}</div>
                    </div>
                    <div class="status-badge ${status.includes('Selesai') || status.includes('Diproses') ? 'status-active' : 'status-inactive'}">
                        <i class="fas ${status.includes('Selesai') || status.includes('Diproses') ? 'fa-circle-check' : 'fa-circle-xmark'} me-1"></i>${status}
                    </div>
                </div>

                <div class="detail-row">
                    <div class="label"><i class="fas fa-layer-group me-2"></i>Batch Produksi</div>
                    <div class="value">${batch}</div>
                </div>

                <div class="detail-row">
                    <div class="label"><i class="fas fa-calendar me-2"></i>Tanggal Produksi</div>
                    <div class="value">${tanggal}</div>
                </div>

                <div class="detail-row">
                    <div class="label"><i class="fas fa-target me-2"></i>Target Produksi</div>
                    <div class="stok-highlight">
                        ${parseFloat(target).toLocaleString('id-ID')} unit
                    </div>
                </div>

                <div class="detail-row">
                    <div class="label"><i class="fas fa-check-circle me-2"></i>Hasil Produksi</div>
                    <div class="value">${parseFloat(hasil).toLocaleString('id-ID')} unit</div>
                </div>

                <div class="detail-row">
                    <div class="label"><i class="fas fa-dollar-sign me-2"></i>Biaya Produksi</div>
                    <div class="stok-highlight">
                        ${biaya}
                    </div>
                </div>
        ...
        document.addEventListener('click', function(event) {
            if (event.target.closest('.delete-btn')) {
                ...
                if (typeof Swal !== 'undefined') {
                    ...
                    if (confirm(`Apakah Anda yakin ingin menghapus produksi ${nama}?`)) {
        ...
        function confirmDelete(id, nama, url, buttonElement) {
            Swal.fire({
                ...
                html: `Apakah Anda yakin ingin menghapus produksi <strong>${nama}</strong>?<br><br><small class="text-muted">Data yang dihapus tidak dapat dikembalikan.</small>`,
                icon: 'warning',
